        ...
        <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-mono selection:bg-red-500">
            ...
            <main className="max-w-5xl mx-auto px-6 py-20">
                <header className="mb-16 space-y-4">
                    ...
                    <p className="text-muted text-[10px] tracking-[0.3em] uppercase font-bold">Confidential History {"//"} {roasts.length} Records Detected</p>
                </header>

                {loading ? (
                    <HistoryPageSkeleton />
                ) : roasts.length === 0 ? (
                    <div className="text-center py-40 border border-border bg-card transition-colors duration-300">
                        <p className="text-muted uppercase tracking-widest text-[10px] font-black">No diagnostic records found.</p>
                        ...
                        {roasts.map((roast) => (
                            ...
                            >
                                <div className="flex items-center gap-8">
                                    <div className="w-12 h-12 bg-card border border-border flex items-center justify-center group-hover:bg-red-500/10 transition-colors">
                                        <FileText size={20} className="text-muted group-hover:text-red-500" />
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-black uppercase tracking-tight group-hover:text-red-500 transition-colors">{roast.deck_name}</h3>
                                        <p className="text-[9px] text-muted uppercase font-bold tracking-widest mt-1">
                                ...
                                <div className="flex items-center gap-10">
                                    <div className="flex flex-col items-end">
                                        <span className="text-2xl font-black italic text-muted group-hover:text-foreground transition-colors">{roast.result_json.fundability_score}%</span>
                                        <span className="text-[8px] uppercase tracking-widest text-muted font-black transition-colors duration-300">Fundability</span>
                                    </div>
                                    <div className="h-10 w-px bg-border transition-colors duration-300" />
                                    {roast.pdf_unlocked ? (
            ...
            <footer className="p-10 border-t border-border text-center transition-colors duration-300">
                <p className="text-[8px] text-muted uppercase tracking-widest font-black">DeckSlayer Protocol {"//"} Secure Archives</p>
            </footer>
        </div>
    );
}
